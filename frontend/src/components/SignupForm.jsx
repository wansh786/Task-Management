...
const SignupForm = () => {
  ...
  return (
    <>
      <form className='bg-primary m-auto my-16 max-w-[500px] p-8 bg-white border-2 shadow-md rounded-md'>
        {loading ? (
          <Loader />
        ) : (
          <>
            ...
            <button className='bg-teal-800 text-white px-4 py-2 font-medium hover:bg-primary-dark' onClick={handleSubmit}>Submit</button>

            <div className='pt-4'>
              <Link to="/login" className='text-blue-800'>Already have an account? Login here</Link>
            </div>
          </>
        )}
